...
void vmsProcessesInjector::DoJob()
{
	...
	for (int i = 0; i < pl.getProcessCount (); i++)
	{
		...
		for (size_t j = 0; j < m_vtstrNamesOfProcesesToMonitor.size (); j++)
		{
	...
	for (size_t j = 0; j < vdwPIDs.size (); j++)
	{
		DWORD dwPID = vdwPIDs [j];

		for (size_t k = 0; k < m_vProcesses.size (); k++)
		{
	...
	for (j = 0; j < m_vProcesses.size (); j++)
	{
		...
		for (size_t k = 0; k < vProcesses.size (); k++)
		{

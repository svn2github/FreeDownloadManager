...
// Note this function is for compatibility with older versions and
// is not used in the new design where the output pin upstream
// negotiates directy with the input pin downstream
...
//
//  Dummy - we don't ever decide an allocator ourselves - we
//  let the upstream filter talk almost directly with the
//  downstream input pin
//
HRESULT
CTransInPlaceOutputPin::DecideAllocator(IMemInputPin *pPin, IMemAllocator **ppAlloc)
{
    //  Allocator will be decided during input pin reconnect
    return S_OK;
} // DecideAllocator

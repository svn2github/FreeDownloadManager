...
#define NS_DECL_NSITXTTOHTMLCONV \
  NS_IMETHOD SetTitle(const char16_t * text); \
  NS_IMETHOD PreFormatHTML(bool value); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSITXTTOHTMLCONV(_to) \
  NS_IMETHOD SetTitle(const char16_t * text) { return _to SetTitle(text); } \
...
#define NS_FORWARD_SAFE_NSITXTTOHTMLCONV(_to) \
  NS_IMETHOD SetTitle(const char16_t * text) { return !_to ? NS_ERROR_NULL_POINTER : _to->SetTitle(text); } \

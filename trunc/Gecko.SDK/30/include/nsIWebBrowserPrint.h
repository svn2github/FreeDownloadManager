...
#define NS_FORWARD_NSIWEBBROWSERPRINT(_to) \
  ...
  NS_IMETHOD EnumerateDocumentNames(uint32_t *aCount, char16_t * **aResult) { return _to EnumerateDocumentNames(aCount, aResult); } \
...
#define NS_FORWARD_SAFE_NSIWEBBROWSERPRINT(_to) \
  ...
  NS_IMETHOD EnumerateDocumentNames(uint32_t *aCount, char16_t * **aResult) { return !_to ? NS_ERROR_NULL_POINTER : _to->EnumerateDocumentNames(aCount, aResult); } \

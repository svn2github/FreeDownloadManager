...
#define NS_DECL_NSIX509CERT3 \
  ...
  NS_IMETHOD GetAllTokenNames(uint32_t *length, char16_t * **tokenNames); 
...
#define NS_FORWARD_NSIX509CERT3(_to) \
  ...
  NS_IMETHOD GetAllTokenNames(uint32_t *length, char16_t * **tokenNames) { return _to GetAllTokenNames(length, tokenNames); } 
...
#define NS_FORWARD_SAFE_NSIX509CERT3(_to) \
  ...
  NS_IMETHOD GetAllTokenNames(uint32_t *length, char16_t * **tokenNames) { return !_to ? NS_ERROR_NULL_POINTER : _to->GetAllTokenNames(length, tokenNames); } 
...
/* void getAllTokenNames (out unsigned long length, [array, size_is (length), retval] out wstring tokenNames); */
NS_IMETHODIMP nsX509Cert3::GetAllTokenNames(uint32_t *length, char16_t * **tokenNames)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
...
#define NS_DECL_NSICERTVERIFICATIONRESULT \
  NS_IMETHOD GetUsagesArrayResult(uint32_t *verified, uint32_t *count, char16_t * **usages); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSICERTVERIFICATIONRESULT(_to) \
  NS_IMETHOD GetUsagesArrayResult(uint32_t *verified, uint32_t *count, char16_t * **usages) { return _to GetUsagesArrayResult(verified, count, usages); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSICERTVERIFICATIONRESULT(_to) \
  NS_IMETHOD GetUsagesArrayResult(uint32_t *verified, uint32_t *count, char16_t * **usages) { return !_to ? NS_ERROR_NULL_POINTER : _to->GetUsagesArrayResult(verified, count, usages); } 

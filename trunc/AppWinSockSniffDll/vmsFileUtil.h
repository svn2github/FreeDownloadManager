...
public:
	...
	static string GetPathFromPathName (LPCSTR pszPathName);
	...
	static void ReadHeader (vmsFile& file, string& strDesc, WORD& wVersion);
	
	static void WriteHeader (vmsFile& file, LPCSTR pszDesc, WORD wVersion);
	
	static void ReadString (vmsFile& file, string &str);
...
inline void vmsFILE_READ(vmsFile& file, string& str) {vmsFileUtil::ReadString (file, str);}
